...
// handleCreateProposal — архитектор пишет предложение в ЧУЖОЙ workspace (origin=proposal,
// вне роадмапа). actor -> proposed_by; source_ws — key своего воркспейса (бэклинк).
func (a *API) handleCreateProposal(w http.ResponseWriter, r *http.Request) {
	...
	n, err := a.svc.CreateProposal(r.Context(), r.PathValue("ws"), service.CreateProposalInput{
		...
		SourceWs: body.SourceWs, Actor: actorOf(r),
	})
	if err != nil {
		a.writeServiceError(w, r, err)
		return
...
// handleAcceptProposal — приземлить предложение в роадмап (origin->native + parent/status).
// Тело опционально: без него parent=корень, status=нет.
func (a *API) handleAcceptProposal(w http.ResponseWriter, r *http.Request) {
	...
	n, err := a.svc.AcceptProposal(r.Context(), r.PathValue("key"), service.AcceptInput{
		ParentID: body.ParentID, StatusID: body.StatusID, Actor: actorOf(r),
	})
	if err != nil {
		a.writeServiceError(w, r, err)
		return
...
// handleDeclineProposal — отклонить предложение (canceled + коммент; origin остаётся proposal).
